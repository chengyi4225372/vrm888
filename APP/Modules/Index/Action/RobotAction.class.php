	...
	Class RobotAction extends CommonAction{
        ...
        public function buy(){
            ...
            if($id == $faquan) {
                ...
                $jinbi = getMemberField('money2');
                if ($jinbi < $data['price']) {
                    alert('账户余额不足,请先进行充值', U('wallet/onlinerecharge'));
                }

                M("member")->where(array('username' => session('username')))->setDec('money2', $data['price']);
                ...
                if (M('order')->add($map)) {
                    ...
                    if ($parentcount > 0) {
                        ...
                        M("member")->where(array('username' => $parent))->setInc('money', $data['price']*$one/100);
                        account_log($parent, $data['price']*$one/100, '1级购买奖励', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent))->setInc('money', $data['price']*$ones/100);
                        account_log($parent, $data['price']*$ones/100, '1级购买奖励', 1, 2, 1, 0, $username);
                    }

                    if ($parentcount1 > 0) {
                        M("member")->where(array('username' => $parent1))->setInc('money', $data['price']*$two/100);
                        account_log($parent1, $data['price']*$two/100, '2级购买奖励', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent1))->setInc('money', $data['price']*$twos/100);
                        account_log($parent1, $data['price']*$twos/100, '2级购买奖励', 1, 2, 1, 0, $username);
                    }
                    if ($parentcount2 > 0) {
                        M("member")->where(array('username' => $parent2))->setInc('money', $data['price']*$three/100);
                        account_log($parent2, $data['price']*$three/100, '3级购买奖励', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent2))->setInc('money', $data['price']*$threes/100);
                        account_log($parent2, $data['price']*$threes/100, '3级购买奖励', 1, 2, 1, 0, $username);
                    }
                }
                $user_id = session('mid');
                    ...
                    for($i=1;$i<=$daishu;$i++){
                        ...
                        if(!empty($p_userinfo)){
                            if($p_userinfo['level'] == 1 ){//判断是否是城主
                                $buyjj=C('buyjj');
                                M('member')->where("id = {$p_id}")->setInc("money",$data['price']*$buyjj/100);

                                account_log($p_userinfo['username'],$data['price']*$buyjj/100,$i.'代/ID号'.$user_id,1,2);
                            }
