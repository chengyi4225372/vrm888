	...
	*/
	class MemberAction extends CommonAction{
		...
		public function addJinbiHandle(){
			...
			if($jinbi>0){
		        M('member')->where(array('id'=>$userid))->setInc('money2',$jinbi);
				//写入充值记录
				$data            = array();
				$data['member']  = $member['username'];
				$data['adds']     = $jinbi;
				$data['balance'] = $member['money2'] + $jinbi;
				$data['addtime'] = time();
				$data['desc']    = '平台充值';
				M('jinbidetail')->add($data);
				$this->success('充值成功！',U(GROUP_NAME.'/Member/check'));
			}elseif($jinbi<0){
				$oldjinbi =$member['money2'];
				$jinbi =abs($jinbi);
                 M('member')->where(array('id'=>$userid))->setDec('money2',$jinbi);
				$data = array();
				$data['member']  = $member['username'];
				$data['reduce']  = $jinbi;
