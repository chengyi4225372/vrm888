		...
		*/
        ...
        public function buy(){
            if (IS_AJAX){
                ...
                $data['user'] = I('post.user','','trim');
                $data['category'] = I('post.category','','trim');
                $data['goods'] = I('post.goods','','trim');
                $data['url'] = I('post.url','','trim');
                $data['price'] = I('post.price',0,'intval');
                $data['number'] = I('post.count',0,'intval');
                $data['mark'] = I('post.mark','','trim');
                if (empty($data['goods']) || empty($data['category']) || empty($data['url']) || empty($data['price']) | empty($data['number']))
                {
                    $this->ajaxReturn(['code'=>0,'msg'=>'参数有误!']);
                    exit();
                }
                $res = M("neworder")->add($data);
                if (empty($res))
                {
                    $this->ajaxReturn(['code'=>0,'msg'=>'下单失败!']);
                } else {
                    $this->ajaxReturn(['code'=>1,'msg'=>'下单成功!']);
                }
            }
        }
