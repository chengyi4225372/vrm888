	...
	$.fn.multiselect=function(options){
		...
		this.each(function(){
			...
			_t.find('option').each(function(i){
				...
				if (_this.attr('icon')) {
					newOptions += '<img src="' + _this.attr('icon') + "\"><li data-value='"+_thisValue+"' "+ selectedOption +" >"+ _thisHtmlValue + selected +"</li>";
				} else {
			...
			// } else {
			// 	newSelectTitle = '<lable class="newSelectTitle">'+a+'<i class="fa fa-angle-down arrow"></i></lable>';
			// }
			var leftText = leftTitle ? leftTitle : options.leftTitle;
			var rightText = rightTitle ? rightTitle : options.rightTitle;
			newSelectTitle = '<lable class="newSelectTitle"><div class="newSelectDisplay"><span class="leftSpan">' + leftText
				+ '</span><div class="rightSpan"><span >' + rightText + '</span> <i class="fa fa-angle-down arrow" style="margin-left: 5px"></i></div> </div></lable>';
			...
			newOptions = '<ul class="newOptions" style="width:'+options.optionWidth+';max-height:' + newOptionsHeight + 'px;overflow-y:auto;">'+newOptions+sArea+'</ul>';
			...
			_thisNewSelect.find('.newOptions li').on(options.optionEventType,function(){
				...
				//判断是否多选
				if(!options.multiple){
					_thisNewSelect.find('.newSelectTitle .leftSpan').html(thisLi.html());
					_thisNewSelect.find('.newSelectTitle .rightSpan').html('');
			...
			if(options.multiple){
				...
				_thisNewSelect.find('.sArea button').on('click',function(event){
					...
					if(selectLi.length > 0){
						...
						_thisNewSelect.find('.newSelectTitle leftSpan').html(divValue);
					}else{
						_thisNewSelect.find('.newSelectTitle leftSpan').html(options.selectedHtmlValue);
					}
